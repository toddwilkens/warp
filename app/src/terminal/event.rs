...
pub enum Event {
    ...
    /// Telemetry marker emitted after the resume fast-path completes
    /// (`TerminalModel::resume_warpify_session`). The actual warpify UI
    /// activation rides on the synthesized `InitShell`+`Bootstrapped`
    /// flow; this unit variant exists purely so the model-events
    /// dispatcher can distinguish "fast-path resume" from a real subshell
    /// bootstrap when firing telemetry. See isidore-infra#560.
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::ResumeWarpifySession => write!(f, "ResumeWarpifySession"),
            Event::InitSsh(event) => {
